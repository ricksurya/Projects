...
class Maker {
    ...
    void readFileAges(String fileInfoName) {
        ...
        try {
            ...
            Scanner inp = new Scanner(fileInfo);
            ...
            error("Could not find fileInfo: %s", fileInfoName);
        }
    }
    ...
    class MakeTraversal extends DepthFirstTraversal {
        ...
        protected void processSuccessors(int v) {
            ...
            for (int i = 0; i < _depends.outDegree(v); i++) {
                if (processSuccessor(v, _depends.getSuccessor(v, i))) {
                    s.add(_depends.getSuccessor(v, i));
                }
            }
            Collections.reverse(s);
            s.forEach(_fringe::add);
        }
