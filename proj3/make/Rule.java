...
class Rule {
    ...
    void addDependency(Rule dependent) {
        if (!_depends.contains(_vertex, dependent._vertex)) {
            _depends.add(getVertex(), dependent.getVertex(),
                    _depends.outDegree(getVertex()));
        }
    }
    ...
    void addCommands(List<String> commands) {
        if (!_commands.isEmpty() && !commands.isEmpty()) {
            throw new IllegalStateException("Commands is non-empty, "
                    + "but I already have a non-empty command set");
        } else {
    ...
    private void checkFinishedDependencies() {
        for (int s : _depends.successors(_vertex)) {
            if (_depends.getLabel(s).isUnfinished()) {
                Error:error("Error:Dependencies aren't built");
            }
        }
    }
    ...
    private boolean outOfDate() {
        if (getTime() == null) {
            ...
            for (int s : _depends.successors(_vertex)) {
                if (_depends.getLabel(s).getTime() > this.getTime()
                        || _depends.getLabel(s).getTime() == null) {
                    return true;
                }
            }
            return false;
