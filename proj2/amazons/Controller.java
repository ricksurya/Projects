...
final class Controller {
    ...
    /**Command do move.
     * @param s The matcher.*/
    private void doMove(Matcher s) {
        if (_winner != null) {
            System.out.println("misplaced move");
            return;
        } else {
            String l = s.group();
            board().makeMove(Move.mv(l));
    ...
    /**Changes to manual.
     * @param s The matcher.*/
    private void doManual(Matcher s) {
        String l = s.group();
        String player = l.substring(6, l.length()).trim();
    ...
    /**Changes to auto.
     * @param s the Matcher.*/
    private void doAuto(Matcher s) {
        String l = s.group();
        String player = l.substring(4, l.length()).trim();
        if (player.equals("black")) {
            _black = _autoPlayerTemplate.create(BLACK, this);
        } else {
